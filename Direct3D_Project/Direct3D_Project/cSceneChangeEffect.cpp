...
cSceneChangeEffect::cSceneChangeEffect()
	: m_pPrevRenderTexture(NULL), m_pNextScene(NULL),
	m_pNextRenderTexture(NULL), m_pPrevScene(NULL)
{
}
...
void cSceneChangeEffect::ChangeStart(cScene* pPrevScene, cScene* pNextScene, float chageTime)
{
	//교체 효과에 사용될 이전 씬과 다음씬을 기억
	m_pPrevScene = pPrevScene;
	m_pNextScene = pNextScene;

	//시간 초기화
	m_TotalTime = chageTime;
	m_DeltaTime = 0.0f;
	m_TimeFactor = 0.0f;
